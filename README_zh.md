...
本节收集在对当前实现进行代码审查时发现的问题，按严重程度分组。这些问题均尚未修复 —— 它们是对贡献者开放的待决问题。

### Bug / 逻辑问题

- **`Span::of_ms` 对负数的处理** —— 对于负数输入，`ps % 1000L` 可能为负，且借位 / 进位的规范化是不对称的。
- **`Ptime::normalize` 与 `Span::normalize` 不一致** —— 两者采用的进位 / 借位算法语义不同；同一个值经由不同路径可能被规范化为不同的表示。
- **`Span::neg` 未规范化** —— 取负直接翻转符号而没有调用 normalize，可能留下非法（未规范化）的表示。
- **`Span::scalar` 未防范溢出** —— Int64 乘法溢出未做检查。
- **`Span::of_float` 依赖不完善的 normalize** —— 负小数部分的处理依赖于上文描述的不一致的规范化。

### 缺失功能 / API 不一致

- **RFC 3339 输出丢失亚秒精度** —— `to_rfc3339` 只输出秒级精度，与该库宣称的皮秒精度卖点相矛盾。
- **不支持闰秒** —— `of_ymd_hms` 将 `second` 限定在 `0..59`，且未说明闰秒限制。
- **缺少 `from_rfc3339` 解析器** —— 只有 `to_rfc3339`，无法把 RFC 3339 字符串解析回 `Ptime`。
- **`mul` / `scalar` 命名不一致** —— README 声称有 `mul` 操作，但实现暴露的是 `scalar`。
- **`Ptime` 结构体注释具有误导性** —— 文档中的 `picoseconds` 范围与 normalize 的目标不一致。

### 工程 / 维护

- **没有公开的 normalize / 不变式检查** —— normalize 是私有的，调用方无法保证所有实例都持有规范形式。
- **测试覆盖不足** —— 缺少针对负数、边界值与溢出的回归测试。
